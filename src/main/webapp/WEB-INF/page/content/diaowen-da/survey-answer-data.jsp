	...
	<div class="creatgeSurveyStepBody">
		<div class="creatgeSurveyStepContent bodyCenter">
			<ul class="createSsUl">
				...
				<li><a href="${ctx }/design/my-collect/execute?surveyId=${surveyId }"  class="clickHideMenu csscStep csscStep5"><span class="csscStepLeft" style="width: 36px">&nbsp;</span><span class="csscStepCenter">数据收集</span><span class="csscStepRight" >&nbsp;</span></a>
					<div class="a-w-sel">
		            	<div class="w-sel" style="margin-top: 4px;">
		                	<div class="selc">
		                    	<div class="selcc tbtag">
		...
		<div class="main-tabs-content bodyCenter">
			<div class="tab-content">
				<div class="tab-content-collectTab">
					...
					<a href="${ctx }/da/my-survey-answer/execute?surveyId=${surveyId}" class="collectTab tabItem_3 active"><span class="collectTabItemLeft">&nbsp;</span><span>原始数据</span></a>
				</div>
			</div>
		</div>
	</div>
							...
							<div style="padding-top: 15px;text-align: center;">
									<div class="btn-group">
										<c:if test="${page.pageNo > 1}">
											<a href="${ctx }/da/my-survey-answer/execute?surveyId=${surveyId }&page.pageNo=${page.pageNo-1}" class="btn btn-default">&lt;</a>
										</c:if>
										<c:if test="${page.startpage > 1}">
											<a href="${ctx }/da/my-survey-answer/execute?surveyId=${surveyId }&page.pageNo=1" class="btn btn-default">1</a>
										...
										<c:forEach begin="${page.startpage }" end="${page.endpage }" var="en">
											<c:choose>
												<c:when test="${page.pageNo eq en }"><a href="${ctx }/da/my-survey-answer/execute?surveyId=${surveyId }&page.pageNo=${en }" class="btn btn-default" style="background: #D3DEED;">${en }</a></c:when>
												<c:otherwise><a href="${ctx }/da/my-survey-answer/execute?surveyId=${surveyId }&page.pageNo=${en}" class="btn btn-default">${en }</a></c:otherwise>
											</c:choose>
										</c:forEach>
										<c:if test="${page.totalPage > (page.endpage)}">
											...
											<a href="${ctx }/da/my-survey-answer/execute?surveyId=${surveyId }&page.pageNo=${page.totalPage}" class="btn btn-default">${page.totalPage }</a>
										</c:if>
