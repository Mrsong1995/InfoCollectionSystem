...
.dialogBtn1{
	...
	font-weight:bold;
  	cursor: pointer;
	dialogMessageButton ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only  	padding: 8px 10px;
}
							...
							<div style="padding: 5px;color:#666565; ">
								<table class="contacts-table" width="100%" cellpadding="0" cellspacing="0">
									...
									<tr>
										...
										<td align="left"><a target="_blank" href="${ctx }/dwsurvey/dws-answer/execute/${en.sid }.html" class="titleTag">${en.surveyName }</a></td>
										...
										<td align="left">
											<div class="btn-group surveyLeftBtnGroup">
											  <a class="btn btn-default" href="${ctx }/design/my-survey-design/execute?surveyId=${en.id}" title="设计"data-toggle="tooltip" data-placement="top" ><i class="fa fa-pencil-square-o"></i></a>
								...
								<div style="padding-top: 15px;text-align: center;">
									<div class="btn-group">
										<c:if test="${page.pageNo > 1}">
											<a href="${ctx }/design/my-survey/list?page.pageNo=${page.pageNo-1}" class="btn btn-default">&lt;</a>
										</c:if>
										<c:if test="${page.startpage > 1}">
											<a href="${ctx }/design/my-survey/list?page.pageNo=1" class="btn btn-default">1</a>
										</c:if>
										<c:forEach begin="${page.startpage }" end="${page.endpage }" var="en">
											<c:choose>
												<c:when test="${page.pageNo eq en }"><a href="${ctx }/design/my-survey/list?page.pageNo=${en }" class="btn btn-default" style="background: #D3DEED;">${en }</a></c:when>
												<c:otherwise><a href="${ctx }/design/my-survey/list?page.pageNo=${en}" class="btn btn-default">${en }</a></c:otherwise>
											</c:choose>
										</c:forEach>
										<c:if test="${page.totalPage > (page.endpage)}">
											<a href="${ctx }/design/my-survey/list?page.pageNo=${page.totalPage}" class="btn btn-default">${page.totalPage }</a>
										</c:if>
