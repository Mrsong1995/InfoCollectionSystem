...
public class Page<T> extends PageRequest implements Iterable<T> {
	...
	public Page(IPageToPage<T> iPage) {
		this.pageNo = (int) iPage.getCurrent();
		this.pageSize = (int) iPage.getSize();
		this.countTotal = iPage.isCountTotal();
		this.orderBy = iPage.getOrderBy();
		this.orderDir = iPage.getOrderDir();
		this.result = iPage.getRecords();
		this.totalItems = iPage.getTotal();
	}
